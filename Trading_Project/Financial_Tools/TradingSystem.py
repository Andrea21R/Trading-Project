...
class TradingSystem(object):
    ...
    def _ranking_method_choice(self, ranking_method, best,
                                     ranking_periods=None, last=True, rolling=None):

        ranker = Tools.ranking.Ranker(dataframe=self.dataframe, is_returns=self._is_returns)

        if ranking_method == 'returns':
            assets = ranker.ranking_by_returns(ranking_periods=ranking_periods, best=best, last=last)
        elif ranking_method == 'ma':
            assets = ranker.ranking_by_moving_average(rolling=rolling, best=best)
        elif ranking_method == 'returns, ma':
            assets = ranker.ranking_by_returns_and_ma(rolling=rolling, ranking_periods=ranking_periods,
                                                      best=best, last=last)
        else:
    ...
    def portfolio_building(self, starting_value,
                           ranking_method, best,
                           ranking_periods=None, last=True, rolling=None):


        assets = TradingSystem._ranking_method_choice(self,
                                                      ranking_method=ranking_method, best=best,
                                                      ranking_periods=ranking_periods, last=last,
                                                      rolling=rolling
                                                      )
        self.portfolio = Tools.Portfolio.Portfolio(starting_value=starting_value, assets=assets, date=self.dataframe.index[-1])

    def portfolio_rebalancing(self, new_prices: DataFrame, is_returns, time_passed,
                              ranking_method, best,
                              ranking_periods=None, last=True, rolling=None,
                              bid_ask_spread=None
                              ):
        ...
        self.portfolio.rebalancing(new_assets=new_assets, new_prices=new_prices, time_passed=time_passed,
