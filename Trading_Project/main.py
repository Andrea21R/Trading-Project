...
# Get the data
Downloader = BinanceDownloader(path.URL,
                               start_name_URL=path.start_name_URL,
                               end_name_URL=path.end_name_URL,
                               folder_path=path.folder_path
                               )
price = Downloader.create_specific_dataframe(target_column='close',
                                             index_name='date',
                                             reverse=True
                                             )
# clean the data:
...
# building portfolio
trading_system.portfolio_building(starting_value=1_000_000,
                                  ranking_method='returns, ma',
                                  best=best,
                                  ranking_periods=24*7,
                                  rolling=24*30
                                  )
# data visualization
...
for week in dataset[1:]:
    trading_system.portfolio_rebalancing(new_prices=week,
                                         is_returns=False,
                                         time_passed=7*24,
                                         ranking_method='returns, ma',
                                         best=best,
...
plt.figure()
plt.plot(dates, normalized_value, marker='o', linewidth=2)
plt.grid()
plt.title("PORTFOLIO PATH",  fontweight="bold")
plt.figtext(0.31, 0.8, f'Starting Value :  $ {round(trading_system.portfolio.history_value[0])}',
            fontweight='bold')
plt.figtext(0.31, 0.775, f'Final Value     :  $ {round(trading_system.portfolio.history_value[-1])}',
            fontweight='bold')
plt.figtext(0.31, 0.75, f'TOTAL RETURN  : {round(trading_system.portfolio.history_value[-1]/trading_system.portfolio.history_value[0]*100,2)} %',
            fontweight='bold')
...
plt.title('PROFIT/LOSS TRADING STRATEGY',
          fontweight='bold')
plt.xticks(rotation=50)
